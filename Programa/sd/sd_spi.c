...
/*********************************************************************
 * Function:         unsigned int getcSPI(void)
 *
 * PreCondition:    None
 * Input:           None
 * Output:          the contents of SPIxBUF register in byte/hword/word format
 * Side Effects:    None
 ...
 ********************************************************************/
unsigned int getcSPI(void)
{
	while(!DataRdySPI());		// wait receive data available
	return ReadSPI();
}

/*********************************************************************
 * Function:        unsigned int getsSPI(unsigned int length, unsigned int *rdptr,
 ...
 *                  cleared by the user software.
 ********************************************************************/
unsigned int getsSPI(unsigned int length, unsigned int *rdptr, unsigned int spi_data_wait)
{
	int				wait;
	unsigned int	rxData;
...
/*********************************************************************
 * Function:        void putsSPI(unsigned int length, unsigned int *wrptr)
 *
 * PreCondition:    None
 ...
 * 					current SPI mode!
 ********************************************************************/
void putsSPI(unsigned int length, unsigned int *wrptr)
{
    ...
    {
		while(TxBufFullSPI());  /* wait until there's room in the buffer */
		if(conBits.MODE32)
		{
			SPIxBUF=*wrptr++;
		}
...
BYTE MDD_SDSPI_MediaDetect (void)
{
	...
	if (SPIENABLE == 0)
	{
        ...
        do
        {
            ...
            SD_CS = 1;
            WriteSPIM(0xFF);   //Send some "extraneous" clock pulses.  If a previous
    ...
    switch(info->bStateVariable)
    {
        ...
        case ASYNC_READ_NEW_PACKET_READY:
            ...
            if(ioInfo.dwBytesRemaining != 0x00000000)
            {
                ...
                localPointer = ioInfo.pBuffer;
                localCounter = ioInfo.wNumBytes;
...
BYTE MDD_SDSPI_AsyncWriteTasks(ASYNC_IO* info)
{
    ...
    switch(info->bStateVariable)
    {
        ...
        case ASYNC_WRITE_TRANSMIT_PACKET:
            ...
            do
            {
                SPIxBUF = *localPointer++;
                localCounter--;
                while(!SPIRBF);
                data_byte = SPIxBUF; //Dummy read to clear SPIRBF
            }while(localCounter);         
...
void __attribute__((optimize("-O0"))) Delayms(BYTE milliseconds)
{
    BYTE    ms;
    DWORD   count;
...
/*****************************************************************************
  ...
  Remarks:
    ...
    will initialize SSPBUF/SPIxBUF to 0x00 when reading.  The card expects 0xFF.
  ***************************************************************************************/
BYTE MDD_SDSPI_ReadMedia(void)
{
    putcSPI((BYTE)0xFF);
...
{
    SPIENABLE = 0;
    SPIxSTAT = 0x0000;               // power on state 
...
//    else
    ...
    SPIxCONbits.MSTEN = 1;  /*5*/
    SPIxCONbits.CKP = 0;    /*6*/
    SPIxCONbits.CKE = 1;    /*8*/
    SPIxCONbits.SMP = 1;    /*9*/

    SPIENABLE = 1;          // 15 - enable synchronous serial port
}    
...
/*****************************************************************************
  ...
  Side Effects:
    None.
  Description:
    The InitSPIFastMode function will enable and configure the SPI module.
  Remarks:
    None.
  ***************************************************************************************/
...
void InitSPIFastMode(void)
{
    if (GetSystemClock() <= 20000000) return;
